...
namespace Enyim.Caching
{
	...
	public sealed class MemcachedClient : IDisposable
	{
		...
		public MemcachedClient(IMemcachedClientConfiguration configuration)
		{
			...
			pool = new ServerPool(configuration);
		}
		...
		{
			using (DeleteOperation d = new DeleteOperation(pool, key))
			{
				d.Execute();
		...
		{
			using (GetOperation g = new GetOperation(pool, key))
			{
				g.Execute();
		...
		{
			using (IncrementOperation i = new IncrementOperation(pool, key, amount))
			{
				i.Execute();
		...
		{
			using (DecrementOperation d = new DecrementOperation(pool, key, amount))
			{
				d.Execute();
		...
		{
			return Store(pool, (StoreCommand)mode, key, value, 0, Infinite, DateTime.MinValue);
		}
		...
		{
			return Store(pool, (StoreCommand)mode, key, value, 0, offset, length, Infinite, DateTime.MinValue);
		}
		...
		{
			return Store(pool, (StoreCommand)mode, key, value, 0, validFor, DateTime.MinValue);
		}
		...
		{
			return Store(pool, (StoreCommand)mode, key, value, 0, TimeSpan.MinValue, expiresAt);
		}
		...
		{
			return Store(pool, (StoreCommand)mode, key, value, 0, offset, length, validFor, DateTime.MinValue);
		}
		...
		{
			return Store(pool, (StoreCommand)mode, key, value, 0, offset, length, TimeSpan.MinValue, expiresAt);
		}
		...
		{
			return Store(pool, StoreCommand.Append, key, data, 0, Infinite, DateTime.MinValue);
		}
		...
		{
			return Store(pool, StoreCommand.Prepend, key, data, 0, Infinite, DateTime.MinValue);
		}
		...
		{
			return Store(pool, StoreCommand.CheckAndSet, key, value, cas, Infinite, DateTime.MinValue);
		}
		...
		{
			return Store(pool, StoreCommand.CheckAndSet, key, value, cas, offset, length, Infinite, DateTime.MinValue);
		}
		...
		{
			return Store(pool, StoreCommand.CheckAndSet, key, value, cas, validFor, DateTime.MinValue);
		}
		...
		{
			return Store(pool, StoreCommand.CheckAndSet, key, value, cas, TimeSpan.MinValue, expiresAt);
		}
		...
		{
			return Store(pool, StoreCommand.CheckAndSet, key, value, cas, offset, length, validFor, DateTime.MinValue);
		}
		...
		{
			return Store(pool, StoreCommand.CheckAndSet, key, value, cas, offset, length, TimeSpan.MinValue, expiresAt);
		}
		...
		{
			using (FlushOperation f = new FlushOperation(pool))
			{
				f.Execute();
			}
		}
		...
		{
			using (StatsOperation s = new StatsOperation(pool))
			{
				s.Execute();
		...
		{
			using (MultiGetOperation mgo = new MultiGetOperation(pool, keys))
			{
				mgo.Execute();
